...
public class EditarUsuario extends javax.swing.JFrame {
    private final Usuario editarUsuario;
    private final Main tela;

    public EditarUsuario(Usuario u, Main tela) {
        initComponents();
        ...
        this.tela = tela;
        lNomeUsuario.setText(editarUsuario.getNome());
        lLoginUsuario.setText(editarUsuario.getLogin());
    ...
    private void edit() {
        ...
        if (!Validacao.isLessOrEqualThan(nome, 200)) {
            ...
            senha = md5.md5(senha);
            if(!this.editarUsuario.getNome().equals(nome) && !Validacao.isEmpty(nome))
                this.editarUsuario.setNome(nome);
            if(!this.editarUsuario.getLogin().equals(login) && !Validacao.isEmpty(login))
                this.editarUsuario.setLogin(login);
            if(!this.editarUsuario.getSenha().equals(senha) && !Validacao.isEmpty(senha))
                this.editarUsuario.setSenha(senha);
            if (uc.editar(this.editarUsuario)) {
